...
- For the `assets/img/networks-war-demo.png` drag phase, capture the first frame before advancing the simulation so the thumbnail begins at the exact settled start pose. During the drag, advance the D3 simulation by `1` tick per captured frame so the non-dragged nodes can move naturally without a sudden catch-up jump. Taper `simulation.alphaTarget` and node velocities by `Math.sin(Math.PI * t)` across the forward drag so the graph is settled at the beginning and end, while still responding naturally in the middle; this prevents label/node wiggle when the reversed frames first begin.
