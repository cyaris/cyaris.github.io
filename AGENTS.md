...
- For `assets/img/firework-launcher-demo.png`, treat margin as the horizontal no-launch zone only: explosions may enter the side regions, but intermediate firework launch centers must not. Use a `100px` no-launch margin on both sides of the `360px` canvas, launch the first and last fireworks from horizontal center x=`180`, and launch every intermediate firework at a seeded uniformly random x position from x=`100` through x=`260` so most of each explosion remains inside the icon border.
