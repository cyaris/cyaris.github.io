...
- `assets/img/networks-war-demo.png` is generated as a transparent APNG for the Networks of War project thumbnail. It mirrors the `the_networks_of_war` frontend graph code and static data, using the `Invasion of Afghanistan` war record from `frontend/src/lib/static/graphData.json` (`United States of America`, `Canada`, `United Kingdom`, `France`, and `Australia` on side 1; `Afghanistan` on side 2).
...
- Current `assets/img/networks-war-demo.png` animation settings: `360x360`, transparent RGBA frames, `190` frames, `50ms` per frame, full-frame APNG replacement frames, the same side colors and force-layout constants as `TheNetworksOfWar.svelte`, no node-size descriptor selected, and a forward `96`-frame circular drag path for the `United States of America` node around the fixed primary `Afghanistan` node. After the forward orbit returns to the settled starting position, reverse immediately by appending the completed forward frames in reverse order; do not add a hold/pause frame range between directions.
- For `assets/img/networks-war-demo.png`, run the D3 force simulation to settle the graph before capturing frames. Exclude the chart-edge margin/clamping logic that normally prevents nodes and labels from touching the visible chart border, because this thumbnail has no visible chart border. After simulating and dragging in the no-margin coordinate space, calculate the combined bounds of every node and label across the captured forward frames and apply a final render transform that fits the complete animation inside the `360x360` transparent image with only minimal padding, so labels and nodes may approach but never exceed the image bounds.
- Draw labels for `assets/img/networks-war-demo.png` from the exact `node.participant` strings used by `TheNetworksOfWar.svelte`; do not abbreviate `United States of America` or alter capitalization. Match the component's label-placement rules relative to each node, including `textWidth`, `nameFitsInNode`, inside-label placement, outside-label quadrant selection, `vertical_name_shift`, and `horizontal_name_shift`. The only label-positioning behavior to omit is the final chart-border clamp from `externalLabelX`, because the no-border thumbnail instead fits the complete animated bounds after simulation.
