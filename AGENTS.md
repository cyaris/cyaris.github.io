...
- Current `assets/img/networks-war-demo.png` animation settings: `360x360`, square `360x360` graph layout, transparent RGBA frames, `310` frames, `50ms` per frame, full-frame APNG replacement frames, the same side colors and force-layout constants as `TheNetworksOfWar.svelte`, no node-size descriptor selected, and a forward `96`-frame circular drag path for the `United States` node around the fixed primary `Afghanistan` node. Before capturing the first frame, pin `United States` at angle `0`/3 o'clock and let the D3 force simulation settle the other participants around that start position. After the forward orbit returns to the starting position, capture up to `60` additional force-simulation settle frames, then reverse by appending the completed forward-and-settle frames in reverse order; do not add a static hold/pause frame range between directions.
- For the `assets/img/networks-war-demo.png` drag phase, capture the first frame after the 3 o'clock start layout has settled. Keep `Afghanistan` fixed at the graph center and move only the `United States` fixed drag coordinates around the circular path, but do not pin `Canada`, `United Kingdom`, `France`, or `Australia`; advance the D3 force simulation by `1` tick per captured drag frame so those participants can respond naturally. After the drag returns to 3 o'clock, keep `United States` fixed there while the other nodes settle before reversing.
