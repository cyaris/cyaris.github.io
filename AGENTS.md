...
- For `assets/img/networks-war-demo.png`, run the D3 force simulation to settle the graph before capturing frames. Use `graphLayout.marginSize = 0` so the thumbnail does not reserve the app's visible-chart border padding, but keep label placement independent from the final bounds-fitting step. After simulating and dragging in that no-margin coordinate space, calculate the combined bounds of every rendered frame and apply a final frame-level transform that fits the complete animation inside the `360x360` transparent image with only minimal padding, so labels and nodes may approach but never exceed the image bounds.
- Draw labels for `assets/img/networks-war-demo.png` from the exact `node.participant` strings used by `TheNetworksOfWar.svelte`; do not abbreviate `United States of America` or alter capitalization. Match the component's label-placement rules relative to each node, including `textWidth`, `nameFitsInNode`, inside-label placement, outside-label quadrant selection, `vertical_name_shift`, `horizontal_name_shift`, and `externalLabelX` with the zero-margin graph layout. The final frame-level fit must not feed back into `labelPosition`, `externalLabelX`, or any other label-placement calculation.
