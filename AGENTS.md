...
- Current `assets/img/networks-war-demo.png` animation settings: `360x360`, square `360x360` graph layout, transparent RGBA frames, `190` frames, `50ms` per frame, full-frame APNG replacement frames, the same side colors and force-layout constants as `TheNetworksOfWar.svelte`, no node-size descriptor selected, and a forward `96`-frame circular drag path for the `United States` node around the fixed primary `Afghanistan` node. Before capturing the first frame, initialize the graph with the Networks force/layout setup, then place `United States`, `Canada`, `United Kingdom`, `France`, and `Australia` on equal 72-degree radial steps around `Afghanistan`, with `United States` at angle `0`/3 o'clock. After the forward orbit returns to the starting position, reverse immediately by appending the completed forward frames in reverse order; do not add a hold/pause frame range between directions.
- For the `assets/img/networks-war-demo.png` drag phase, capture the first frame at the exact radial start pose. Store those fixed radial positions, then keep every non-dragged participant fixed there for the full captured drag. Move only the `United States` node along the circular path around `Afghanistan`, set its velocity to zero each captured frame, and recompute link-node midpoints without advancing the force simulation. This keeps the final forward frame pixel-identical to the first frame and makes the immediate reverse begin without node or label wiggle.
...
- Match the component's label-placement rules relative to each node, including `textWidth`, `nameFitsInNode`, inside-label placement, outside-label quadrant selection, `vertical_name_shift`, `horizontal_name_shift`, and `externalLabelX` with the zero-margin graph layout. The final bounds fit must not feed back into `labelPosition`, `externalLabelX`, or any other label-placement calculation. When rasterizing labels directly into fixed-size PNG frames, do not multiply `label.x` or `label.y` by the final render scale; add those component-computed offsets to the transformed node anchor so text spacing stays visually consistent with the Networks of War SVG tool. The thumbnail intentionally adds a small raster-only smoothing layer for the dragged `United States` label: when the source rule changes label side, render a short opacity crossfade between the two source-rule positions instead of moving text through the node, and explicitly copy the first forward label layer to the final forward frame so the label position is identical at the drag start/end and the immediate reverse begins cleanly.
